...
void LocalMapping::CreateNewMapPoints()
{
    ...
    // For stereo inertial case(立体 惯性情况)
    //如果是单目相机的情况下,取30个
    if(mbMonocular)
        nn=30;
        //如何提取相邻关键帧：定义容器,名为vpNeighKFs,存放KeyFrame的指针
    ...
    //设置一个与特征匹配相关的阈值
    float th = 0.6f;
    //定义了一个ORBmatcher类的对象，输入的参数是阈值和True/False
    ORBmatcher matcher(th,false);

    //提取当前关键帧的位姿信息，包括旋转和平移
    ...
    for(size_t i=0; i<vpNeighKFs.size(); i++)
    {
        ...
        //两个指针，分别指向当前关键帧的相机对象，和当前处理的相邻关键帧的相机对象
        ...
        // Search matches that fullfil epipolar constraint,对极约束
        //名为vMatchedIndices,类型为pair<>
        vector<pair<size_t,size_t> > vMatchedIndices;
        //bCoarse由是否是惯性，追踪器和?决定
        bool bCoarse = mbInertial && mpTracker->mState==Tracking::RECENTLY_LOST && mpCurrentKeyFrame->GetMap()->GetIniertialBA2();

        //matcher对象的Search方法完成特征匹配，特征点存放于mpCurrentKeyframe所指向的对象中，返回的特征匹配存放于，vMatchedIndices这个容器中
        matcher.SearchForTriangulation(mpCurrentKeyFrame,pKF2,vMatchedIndices,false,bCoarse);

        //获得当前处理的相邻关键帧的旋转，平移信息
        ...
        // Triangulate each match对每个匹配进行三角测量
        ...
        for(int ikp=0; ikp<nmatches; ikp++)
        {
            ...
            //判读是否为单目相机，单目相机从mvKeysUn中提取关键特征点(当前关键帧)
            ...
            std::cout<<"bRignt1 "<<bRight1<<std::endl;                                                                                  
            ...
            std::cout<<"bRignt2 "<<bRight2<<std::endl;  
            std::cout<<"bStereo1 "<<bStereo1<<std::endl;
            std::cout<<"bStereo2 "<<bStereo2<<std::endl;
            ...
            //Check reprojection error in first keyframe重投影误差
