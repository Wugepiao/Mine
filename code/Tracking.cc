...
Sophus::SE3f Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp, string filename)
{
    ...
    {
        cv::Point2f point = mCurrentFrame.mvKeysUn[i].pt;
        cv::Point2f pt1,pt2;
        int r = 5;
        pt1.x=point.x-r;
        pt1.y=point.y-r;
        ...
        string name = "./output/"+to_string(N_frame)+".png";
        imwrite(name, im);
    ...
    if(mCurrentFrame.imgLeft.empty())
    {
        cout<<"当前空"<<endl;
    }
...
void Tracking::Track()
{
    ...
    else
    {
        ...
        Eigen::Vector3f twwc = mCurrentFrame.GetPose().translation();
        Eigen::Matrix<float,3,3> R = mCurrentFrame.GetPose().rotationMatrix();
        cout<<"--------当前帧位姿 -------"<<endl;
        cout<<R<<endl;
        cout<<twwc.transpose()<<endl;
    }
...
void Tracking::MonocularInitialization()
{
    ...
    {
        std::cout<<"特征点的数量:"<<mCurrentFrame.mvKeys.size()<<endl;
        // Set Reference Frame
    ...
    else
    {
        ...
        cv::Mat img_matches;
        std::vector<cv::DMatch> matches_Dmatches;
        for(int i=0; i<mvIniMatches.size();i++)
        {
            ...
            if(mvIniMatches[i]>=0)
            {
                ...
                cv::DMatch image_mat;
                image_mat.queryIdx = i;
                image_mat.trainIdx = mvIniMatches[i];
                // cout<<"[ "<<i<<","<<mvIniMatches[i]<<"]"<<endl;
                matches_Dmatches.push_back(image_mat);

            }

        }
        if(mInitialFrame.imgLeft.empty())
        {
            cout<<"empty"<<endl;
        }
        cv::drawMatches(mInitialFrame.imgLeft,mInitialFrame.mvKeysUn, mCurrentFrame.imgLeft,mCurrentFrame.mvKeysUn,matches_Dmatches,img_matches);
        string filename = "./matches/"+to_string(mInitialFrame.mnId)+"_"+to_string(mCurrentFrame.mnId)+".png";
        imwrite(filename,img_matches);


        cout<< "Frame "<<mInitialFrame.mnId<<", "<< mCurrentFrame.mnId<<endl;
        if(mpCamera->ReconstructWithTwoViews(mInitialFrame.mvKeysUn,mCurrentFrame.mvKeysUn,mvIniMatches,Tcw,mvIniP3D,vbTriangulated))
        {
            ...
            cout<<"设定初始位姿 "<<Tcw.translation().transpose()<<endl;
            CreateInitialMapMonocular();
        }
       
        cout<<"初始化状态 mState "<<mState<<"-------";
...
{
    //构建ORBmatcher,用于匹配特征点,阈值设置为0.9
    ORBmatcher matcher(0.9,true);

    // Update last frame pose according to its reference keyframe
    //根据上一帧与它的参考关键帧的相对位姿，乘上它参考关键帧的位姿，来更新上一帧的位姿
    ...
    if(nmatches<50)
    {
        ...
        nmatches = matcher.SearchByProjection(mCurrentFrame,mLastFrame,3*th,mSensor==System::MONOCULAR || mSensor==System::IMU_MONOCULAR);
        Verbose::PrintMess("Matches with wider search: " + to_string(nmatches), Verbose::VERBOSITY_NORMAL);

    }
    cout<<" ---- nmatches 2 "<<nmatches;

    int mn=0;
    ORBmatcher matcher2(0.9,true);
    std::vector<cv::Point2f> mvbPrevMatched2;
    mvbPrevMatched2.resize(mLastFrame.mvKeysUn.size());
    for(size_t i=0; i<mLastFrame.mvKeysUn.size(); i++)
    {
        mvbPrevMatched2[i]=mLastFrame.mvKeysUn[i].pt;
    }              
    std::vector<int> mvIniMatches2;

    mn = matcher2.SearchForInitialization(mLastFrame,mCurrentFrame,mvbPrevMatched2,mvIniMatches2,100);
    //2d匹配数量
    cout<<"----MATCH  "<<mn<<endl;

    //添加的
    GeometricCamera* pCamera1 = mLastFrame.mpCamera, *pCamera2 = mCurrentFrame.mpCamera;//获取前后帧对象的相机

    //提取上一帧的位姿信息，包括旋转和平移
    //键帧返回的T矩阵，调用Sophus库的SE3类，并对其实例化，名为sophTcw1，存放当前关键帧返回的T矩阵，<float>表示这个类中使用float类型的数据
    Sophus::SE3<float> sophTcw1 = mLastFrame.GetPose();
    //得到当前关键帧T矩阵的前3行，调用Eigen库的Matix类，对其实例化，名为eigTcw1,运算类型为float
    Eigen::Matrix<float,3,4> eigTcw1 = sophTcw1.matrix3x4();
    //将旋转矩阵放入Rcw1中,block是提取矩阵,(0,0)是从左上角开始,world→camera
    Eigen::Matrix<float,3,3> Rcw1 = eigTcw1.block<3,3>(0,0);
    //求旋转矩阵的转置，即相机坐标系到世界坐标系的旋转啊,camera→world
    Eigen::Matrix<float,3,3> Rwc1 = Rcw1.transpose();
    //提取平移部分
    Eigen::Vector3f tcw1 = sophTcw1.translation();
    //获取相机中心点的位置(x,y,z)
    Eigen::Vector3f Ow1 = mLastFrame.GetCameraCenter();

    //提取当前帧的位姿信息，包括旋转和平移
    //键帧返回的T矩阵，调用Sophus库的SE3类，并对其实例化，名为sophTcw1，存放当前关键帧返回的T矩阵，<float>表示这个类中使用float类型的数据
    Sophus::SE3<float> sophTcw2 = mCurrentFrame.GetPose();

    //得到当前关键帧T矩阵的前3行，调用Eigen库的Matix类，对其实例化，名为eigTcw2,运算类型为float
    ...
    //获取相机中心点的位置(x,y,z)
    Eigen::Vector3f Ow2 = mCurrentFrame.GetCameraCenter();

    for (size_t i = 0; i < mvIniMatches2.size(); i++) {
        int idxPrev = mvIniMatches2[i];  // 上一帧匹配点的索引
        if (idxPrev >= 0) {  // 如果有匹配点
            cv::Point2f ptPrev = mLastFrame.mvKeysUn[idxPrev].pt;  // 上一帧的匹配点坐标
            cv::Point2f ptCurrent = mCurrentFrame.mvKeysUn[i].pt;  // 当前帧的匹配点坐标

            
            cout << "匹配点对 " << i << ": (" 
                << ptPrev.x << ", " << ptPrev.y << ") -> (" 
                << ptCurrent.x << ", " << ptCurrent.y << ")" << endl;

            //将当前处理关键帧和相邻关键帧选中的特征点的二维图像坐标转换为相机坐标系的三维坐标
            Eigen::Vector3f xn1 = pCamera1->unprojectEig(ptPrev);
            Eigen::Vector3f xn2 = pCamera2->unprojectEig(ptCurrent);
            cout << "xn1: (" << xn1[0] << ", " << xn1[1] << ", " << xn1[2] << ")" << endl;
            cout << "xn2: (" << xn2[0] << ", " << xn2[1] << ", " << xn2[2] << ")" << endl;
            ...
            cout << "三维点 x3D: (" 
                ...
                << x3D[2] << ")" << endl;
            if (goodProj) {
                //// 将 3D 点从世界坐标系转换到当前帧的相机坐标系
                const float x2 = Rcw2.row(0).dot(x3D) + tcw2(0);  
                const float y2 = Rcw2.row(1).dot(x3D) + tcw2(1);
                const float z2 = Rcw2.row(2).dot(x3D) + tcw2(2);
                //获得重投影的坐标
                cv::Point2f uv2 = pCamera2->project(cv::Point3f(x2,y2,z2));
                float errX2 = uv2.x - ptCurrent.x;
                float errY2 = uv2.y - ptCurrent.y;
                //计算重投影误差
                float reprojectionError = sqrt(errX2 * errX2 + errY2 * errY2);  // 重投影误差的欧几里得距离
                //输出重投影误差
                cout << "重投影误差: " << reprojectionError << " (特征点: " << i << ")" << endl;
            //     //设置边对象
            //     g2o::VertexSE3Expmap * vSE3 = new g2o::VertexSE3Expmap();
            //     //上边已经获得了sophTcw2(估计的位姿值)
            //     vSE3->setEstimate(g2o::SE3Quat(sophTcw2.unit_quaternion().cast<double>(),sophTcw2.translation().cast<double>()));
            //     //将这个边的序号设置为0
            //     vSE3->setId(0);
            //     vSE3->setFixed(false);
            //     //设置obs存放二维点
            //     Eigen::Matrix<double,2,1> obs;
            //     obs << ptCurrent.x, ptCurrent.y;
            //     //创建 EdgeSE3ProjectXYZOnlyPose 对象
            ...
            // // 设置当前帧的位姿 
            // Eigen::Isometry3d pose_current = eigTcw2;  // 假设这是当前帧的位姿

            // // 设置3D点
            // e->setVertex(0, dynamic_cast<g2o::VertexSE3Expmap*>(v1));  // v1是当前帧的位姿顶点
            // e->setMeasurement(Eigen::Vector2d(ptCurrent.x, ptCurrent.y));  // 2D观测点
            // e->setInformation(Eigen::Matrix2d::Identity());  // 设置信息矩阵，通常为单位矩阵，除非有先验信息
            ...
            // // 输出误差
            // cout << "重投影误差: " << e->error().transpose() << endl;
            }
        }
    }
    ...
    cv::Mat img_matches2;
    std::vector<cv::DMatch> matches_Dmatches2;
    for(int i=0; i<mvIniMatches2.size();i++)
    {

        if(mvIniMatches2[i]>=0)
        {

            const cv::KeyPoint &kpUn = mCurrentFrame.mvKeysUn[i];
            cv::DMatch image_mat;
            image_mat.queryIdx = i;
            image_mat.trainIdx = mvIniMatches2[i];
            // cout<<"[ "<<i<<","<<mvIniMatches2[i]<<"]"<<endl;
            matches_Dmatches2.push_back(image_mat);
        }
    }
    if(mInitialFrame.imgLeft.empty())
    {
    ...
    cv::drawMatches(mLastFrame.imgLeft,mLastFrame.mvKeysUn, mCurrentFrame.imgLeft,mCurrentFrame.mvKeysUn,matches_Dmatches2,img_matches2);
    string filename = "./matches/"+to_string(mLastFrame.mnId)+"_"+to_string(mCurrentFrame.mnId)+"_tracking.png";
    imwrite(filename,img_matches2);
